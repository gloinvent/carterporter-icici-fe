...
@Component({
  selector: 'app-cargo-transfer',
  templateUrl: './cargo-transfer.component.html',
  styleUrls: ['./cargo-transfer.component.scss']
})
export class CargoTransferComponent implements OnInit {
  @Input() type: number;
  ...
  selected_pincode_array = [
    {
      ...
      delivery_airportDesc: "LOCATION AT AIRPORT: 24/7 CarterX kiosk.\n&FOR DEPARTURES: Delivery till terminal entry gates.\n&FOR ARRIVAL: Bags to be dropped at CarterX kiosk at Arrivals village. Opp. Café Coffee Day.\n&LUGGAGE STORAGE: Available&INTER TERMINAL TRANSFER: Available"
    ...
    },
    {
      ...
      delivery_airportDesc: "LOCATION AT AIRPORT: 24/4 CarterX kiosk at Arrivals. Opp. Relay store &FOR DEPARTURES: Order Delivery up to Airline Check-in counters &FOR ARRIVAL:Order Pick up at Luggage Carousel/baggage belts (Domestic and International) &LUGGAGE STORAGE: Available&INTER TERMINAL TRANSFER: N/A",
  ...
  ngOnInit() {
    ...
    localStorage.setItem("secondAirport",this.delivery_airportDesc);
    ...
    this.type_of_services = this.type;
  }
  ...
  selectpickupcity(city) {
    ...
    localStorage.setItem("secondAirport",city.delivery_airportDesc);
    console.log(city)
  }
  ...
  pincodeAirportSelection(){
    for(let i = 0; i<this.selected_pincode_array.length;i++){
      ...
      if(this.cf.get("delivery_city_id").value === this.selected_pincode_array[i].city_id){
        ...
        localStorage.setItem("secondAirport",this.selected_pincode_array[i].delivery_airportDesc);
